...
### Added
- **CloudFront: Configurable Cross-Origin headers**
  - New `crossOriginOpenerPolicy` option in `ResponseHeaderPolicyOptions` (default: `'same-origin'`)
  - New `crossOriginEmbedderPolicy` option in `ResponseHeaderPolicyOptions` (default: `'require-corp'`)
  - Both headers can be customized with different values or disabled by setting to `false`
  - Useful for controlling Cross-Origin isolation for third-party embeds or SharedArrayBuffer usage
