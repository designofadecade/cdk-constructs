...
/**
 * Configuration settings for the token refresh handler
 */
interface Settings {
    cognito: {
        domain: string;
...
/**
 * Response from Cognito token refresh endpoint
 */
...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
    try {
        ...
        const cloudFrontCookies: string[] = [];
        ...
        return {
            ...
            cookies:
                cloudFrontCookies && cloudFrontCookies.length > 0
                    ? [
